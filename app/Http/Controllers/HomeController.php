    ...
    public function index()
    {
        ...
        foreach ($all_locations as $loc_id => $loc_name) {
            ...
            foreach ($dates as $date) {
                ...
                if (!empty($sell)) {
                    $values[] = doubleval($sell->total_sells);
                } else {
                    $values[] = 0;
                }
            }
        ...
        $sells_chart_1  = new VendasChart();
        $sells_chart_1->title('');
        $sells_chart_1->labels($labels);
        $sells_chart_1->label(__('home.total_sells', ['currency' => $currency->code]));
            //Charts::multi('line', 'highcharts')->title(' ')->template('material')->labels($labels)->elementLabel(__('home.total_sells', ['currency' => $currency->code]));
        ...
        foreach ($all_locations as $loc_id => $loc_name) {
            ...
            foreach ($fy_months as $month) {
                ...
                if (!empty($sell)) {
                    $values_data[] = doubleval($sell->total_sells);
                } else {
                    $values_data[] = 0;
                }
            }
        ...
        $sells_chart_2 = new VendasChart();
        $sells_chart_2->title('');
        $sells_chart_2->labels($labels);
        $sells_chart_2->label(__('home.total_sells', ['currency' => $currency->code]));

            //Charts::multi('line', 'highcharts')->title(__(' '))->labels($labels)->template('material')->elementLabel(__('home.total_sells', ['currency' => $currency->code]));
